 ...
 */
public class S3ReadConnectionOnlineTest {
  @Test(enabled = false)
  void testS3ClientWithApacheHttpClient() throws Exception {
    EmbeddedGobblin job = new EmbeddedGobblin("test");
    Assert.assertTrue(job.jobFile(getClass().getResource("/pull/s3-csv.pull").getPath()).run().isSuccessful());
  }

  @Test(enabled = false)
  void testS3WithFileDump() throws Exception {
    EmbeddedGobblin job = new EmbeddedGobblin("test");
    Assert.assertTrue(job.jobFile(getClass().getResource("/pull/s3-filedump.pull").getPath()).run().isSuccessful());
  }
}
