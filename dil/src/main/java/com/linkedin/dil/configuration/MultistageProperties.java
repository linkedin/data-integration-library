...
package com.linkedin.dil.configuration;
...
import com.linkedin.dil.factory.DefaultS3ClientFactory;
...
public enum MultistageProperties {
  ...
  MSTAGE_DATA_EXPLICIT_EOF("ms.data.explicit.eof", Boolean.class) {
    @Override
    public <T> T getDefaultValue() {
      return (T) Boolean.FALSE;
    }
   ...
   * {@link com.linkedin.dil.preprocessor.GunzipProcessor}
   */
   ...
   * default = {@link com.linkedin.dil.factory.ApacheHttpClientFactory}
   */
  MSTAGE_HTTP_CLIENT_FACTORY("ms.http.client.factory", String.class) {
    @Override
    public <T> T getDefaultValue() {
      return (T) "com.linkedin.dil.factory.ApacheHttpClientFactory";
    }
  },
  /**
  ...
  /**
   * use this property to set custom response type explicitly. You can also have multiple accepted types for
   * multi-stage jobs
   * ex: {"Content-Type":"application/x-gzip","Content-Type1":"application/json"}
   */
  MSTAGE_HTTP_RESPONSE_TYPE("ms.http.response.type", JsonObject.class),
  /**
   ...
   * default = {@link com.linkedin.dil.factory.DefaultJdbcClientFactory}
   */
  MSTAGE_JDBC_CLIENT_FACTORY("ms.jdbc.client.factory", String.class) {
    @Override
    public <T> T getDefaultValue() {
      return (T) "com.linkedin.dil.factory.DefaultJdbcClientFactory";
    }
  },
  ...
  /**
   * Payloads are a type of secondary input. Where activations generates
   * work units, payloads don't. In planning phase, payloads are passed
   * as is to work units without actually reading the content. Payload
   * content are only being read when the transmission starts. The payload
   * property, therefore, contains secondary entries, in a JsonArray form.
   * And the array contains records of secondary inputs, which include
   * path and fields, etc.
   */
  ...
  MSTAGE_S3_CLIENT_FACTORY("ms.s3.client.factory", String.class) {
    @Override
    public <T> T getDefaultValue() {
      return (T) "com.linkedin.dil.factory.DefaultS3ClientFactory";
    }
  },
  /**
   ...
   *
   * Schema cleansing parameter is a JsonObject, and it supports the following elements:
   * enabled : true|false
   * pattern: if enabled, it has default value "(\\s|\\$|@)"
   * replacement: if enabled, it has default value "_"
   * nullable: it has default value "false", allowed values are:
   *  1. true: all fields will be forced to be nullable
   *  2. false: the schema infer will try to detect nullability from samples
   *
   *  This configuration has no impact on schema from metadata stores.
   */
  MSTAGE_SCHEMA_CLENSING("ms.schema.cleansing", JsonObject.class),
  /**
  ...
  MSTAGE_SOURCE_SCHEMA_READER_FACTORY("ms.source.schema.reader.factory", String.class) {
    @Override
    public <T> T getDefaultValue() {
      return (T) "com.linkedin.dil.factory.reader.MetastoreReader";
    }
  },
  /**
   ...
   * or other processes.
   *
   * Target schema should be specified through target schema URN.
   * An URN can point to the schema storage location on DataHub, which is
   * the only supported schema storage for now.
   *
   * If target schema is specified through ms.target.schema, then the syntax
   * of the schema string is same as ms.output.schema.
   */
  ...
  MSTAGE_NORMALIZER_BATCH_SIZE("ms.normalizer.batch.size", Long.class) {
    @Override
    public <T> T getDefaultValue() {
  ...
  public <T> T getProp(State state) {
    if (className == Boolean.class) {
      return (T) Boolean.valueOf(state.getPropAsBoolean(config));
    } else if (className == Integer.class) {
      return (T) Integer.valueOf(state.getPropAsInt(config, 0));
    } else if (className == Long.class) {
      return (T) Long.valueOf(state.getPropAsLong(config, 0L));
